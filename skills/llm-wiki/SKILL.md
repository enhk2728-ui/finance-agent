...
name: llm-wiki
description: >
  LLM Wiki knowledge base management — ingest sources into wiki pages, query knowledge, lint health,
  research topics via web search, enrich wikilinks, and analyze the knowledge graph. Use this skill
  whenever the user mentions ingesting sources, adding to the wiki, importing articles, asking about
  wiki content, searching the knowledge base, checking wiki health, running lint, doing deep research,
  adding wikilinks, or analyzing the knowledge graph. Also trigger when the user drops a file into raw/
  and wants it processed, asks a question that should be answered from wiki content, or says anything
  about wiki pages, entities, concepts, or sources. Operations: /ingest, /query, /lint, /research,
  /enrich, /graph.
---
...
- **`/enrich <file>`** — add `[[wikilinks]]` to a wiki page by matching terms against the wiki index.
- **`/graph`** — analyze the knowledge graph: communities, hubs, wanted pages, surprising connections.
...
- Read the vault language from `.llm-wiki/config.toml` → `vault.language` (e.g., "zh" or "en").
- All generated wiki page content (titles, body text, descriptions) MUST be in the configured language.
- Keep English technical terms in original form (Unity, Claude Code, AI Agent, etc.).
- Technical terms may include English in parentheses for clarity.
...
Add `[[wikilinks]]` to a wiki page by matching terms against the wiki index.

### Steps

1. Read the target wiki page.
2. Read `wiki/index.md` to get the full page listing.
3. Identify terms in the page that should link to existing wiki pages.
4. Return a JSON substitution list (do NOT rewrite the page content):
   ```json
   { "links": [{ "term": "exact text in content", "target": "index page name" }] }
   ```
5. Apply precise string replacement — only insert `[[wikilinks]]`, never modify surrounding content.
6. Skip terms already inside `[[wikilinks]]`.
7. Run `llm-wiki sync`.
...
- At most one link per target (first occurrence).
...
1. Run `llm-wiki graph` to get communities, hubs, wanted pages, and orphans.
2. **Interpret communities**: What topic clusters exist? Are they balanced?
3. **Interpret hubs**: Which pages are knowledge nexuses? Do they need updating?
4. **Interpret wanted pages**: Which missing pages are most linked-to? Prioritize creation.
5. **Identify surprising connections**: Pages in different communities that share unexpected relationships.
6. **Detect knowledge gaps**: Areas of the graph that are sparse or disconnected.
7. Suggest next steps: pages to create, areas to research, connections to strengthen.
